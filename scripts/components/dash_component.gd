...
func dash(origin: Vector3, target: Vector3) -> void:
	target = get_dash_destination(origin, target)
	var targets := get_dash_targets(origin, target)
	start_cooldown()
	activated.emit(target, targets)
	
func try_dash(origin: Vector3, target: Vector3) -> bool:
	if can_dash():
		dash(origin, target)
		return true
	return false
