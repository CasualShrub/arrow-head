...
func _ready() -> void:
	...
	vb.add_child(_volume_row("Master", SettingsManager.master_volume, SettingsManager.set_master_volume))
	vb.add_child(_volume_row("Music", SettingsManager.music_volume, SettingsManager.set_music_volume))
	vb.add_child(_volume_row("SFX", SettingsManager.sfx_volume, SettingsManager.set_sfx_volume))
...
# one labelled volume slider wired to a SettingsManager setter (live-apply + save)
